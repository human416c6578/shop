...

{
	...
	register_clcmd("say /funzone","CmdXDMG")
...
public CmdXDMG(id)
{
	...
	if(gOnPlugin == 1)
	{
		...
		{
			new idx = iPlayers[i]
			player_remove_zone(idx)
		}
		client_print(id,print_chat,"[FuN] Zonele au fost sterse cu succes!")
	}
	else
	{
		fileRead(1)
		client_print(id,print_chat,"[FuN] Zonele au fost adaugate cu succes!")
	}
	return PLUGIN_CONTINUE
}
