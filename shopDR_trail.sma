...
public plugin_init() {
	...
	register_item("Trail", "handleTrail", "shopDR_trail.amxx", 15000, TRAIL_ITEMID);
...
public client_putinserver(id) {
	...
	set_task(2.0, "check_trail", id);
}

public check_trail(id)
{
	if(inventory_get_item(id, TRAIL_ITEMID) || isPlayerVip(id))
		g_bHasTrail[id] = true;
}
...
public handleTrail(id){
	...
	g_bHasTrail[id] = true;
	inventory_add(id, TRAIL_ITEMID);
	CC_SendMessage(0, "&x01Jucatorul &x04%s &x01a cumparat &x07trail &x01din shop pentru &x0415.000 &x01de credite!", szName);
	CC_SendMessage(id, "&x01Felicitari, acum ai acces la comanda &x04trail!", szName);
}
