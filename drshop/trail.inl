...
#define ACCESS_LEVEL		ADMIN_LEVEL_G
...
/*public plugin_init() {
	register_plugin("Trail", "1.0", "ATudor")
...
public cmdUserTrail(id, level, cid) {
	...
	if (access(player, ADMIN_IMMUNITY) && id!=player) {
		console_print(id, "You cannot do that to %s, you silly bear!", Name)
		return PLUGIN_HANDLED
	}
	...
	if (equali(colorname, "off")) {
		if (!gl_player_colors[player][0] && !gl_player_colors[player][1] && !gl_player_colors[player][2]) {
			console_print(id, "The %s's trail is already off!", Name)
			return PLUGIN_HANDLED
		}
		kill_trail_task(player)
		console_print(id, "The %s's trail has been removed.", Name)
		client_print(player, print_chat, "Your trail has been removed.")
	} else if (equali(colorname, "random")) {
		do_trail(player, "", "")
		console_print(id, "%s has now a random color trail.", Name)
	} else {
		do_trail(player, colorname, "")
		console_print(id, "%s has now a %s trail.", Name, colorname)
	}
...
/*public SayCmd(id, level, cid) {
	new msg[128], msg[200]H], typenum
	read_msg(msg, 128), Name[MAX_NAME_LENGTH], colname[MAX_NAME_LENGTH], arg2[MAX_NAME_LENGTH], arg3[MAX_NAME_LENGT
	remove_quotes(msg)
	...
	if (equali(msg, "trail", 5)) {
		...
		if (!msg[5]) {
			...
			parse(msg[6], colname, MAX_NAME_LENGTH, arg2, MAX_NAME_LENGTH, arg3, MAX_NAME_LENGTH)
...
public check_position(taskid) {
	new origin[3], id = taskid-TASKID9
 ...
 *****************************************************************************************/
restart_player_trail(id) {
	if (task_exists(TASKID9+id)) {
		remove_task(TASKID9+id)
		kill_trail_msg(id)
		get_user_origin(id, gl_player_position[id])
		set_task(TICK, "check_position", TASKID9+id, "", 0, "b")
		trail_msg(id)
